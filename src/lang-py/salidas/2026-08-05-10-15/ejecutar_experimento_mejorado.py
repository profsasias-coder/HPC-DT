...
def generar_grafico_speedup(tiempos, speedups, output_dir):
    ...
    print(f"Fraccion paralelizable estimada (f): {f:.4f}")
    print(f"Fraccion secuencial (1-f): {1-f:.4f}")
    ...
    # Anotar el valor de f en el grafico
    axes[1].text(0.7, 0.05, f'f = {f:.3f}', transform=axes[1].transAxes,
                 fontsize=10, bbox=dict(facecolor='white', alpha=0.8))
    axes[1].text(0.7, 0.12, f'1-f = {1-f:.3f}', transform=axes[1].transAxes,
                 fontsize=10, bbox=dict(facecolor='white', alpha=0.8))
