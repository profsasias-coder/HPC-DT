...
PROCESOS = [1, 3, 4, 6, 8, 10, 12, 14, 16, 18, 20, 24, 30, 32, 40, 44]   # Ajusta según tu disponibilidad
REPETICIONES = 1  # Número de ejecuciones por cada N (para promediar)
...
def ejecutar_con_procesos(n_procesos):
    ...
    # Solo Hardware
    #cmd = ["mpirun", "-np", str(n_procesos), "--use-hwthread-cpus", BINARIO]

    # Usa los hilos asociados a cada nucleo
    #cmd = ["mpirun", "-np", str(n_procesos), "--use-hwthread-cpus","--map-by hwthread","--bind-to hwthread", BINARIO]

    # Forzar mayor cantidad de procesos en hilos lógicos
    cmd = ["mpirun", "-np", str(n_procesos), "--use-hwthread-cpus", "--oversubscribe", BINARIO]
...
def ajustar_amdahl(procs, speedups):
    ...
    # Solo usar puntos donde el speedup crece (hasta 8 procesos, o hasta que deje de crecer)
    procs_fit = []
    speedups_fit = []
    for p, s in zip(procs, speedups):
        if p <= 8:  # Ajusta según tu punto óptimo observado
            procs_fit.append(p)
...
def main():
    ...
    # Ajustar Amdahl (solo con puntos de crecimiento, ej. hasta 8)
    ...
    print(f"\nFracción paralelizable (Amdahl, ajustada con puntos de crecimiento): fp = {fp:.4f}")
    ...
    print("\nNOTA: La caída del speedup después del punto óptimo se debe al overhead de comunicación y contención de recursos, que no está contemplado por la ley de Amdahl.")
    ...
    # Nota sobre caída por overhead (posición media izquierda)
    axes[1].text(0.05, 0.5, 'Caída por overhead de comunicación\ny contención de recursos', transform=axes[1].transAxes,
                 fontsize=9, bbox=dict(facecolor='yellow', alpha=0.6), ha='left', va='center')
    ...
    with open(reporte, "w") as archivo_reporte:
        ...
        archivo_reporte.write(f"Fracción paralelizable (Amdahl, ajustada con puntos de crecimiento): fp = {fp:.4f}\n")
