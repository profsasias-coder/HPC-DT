...
1. Comparación: datos reales vs modelo descubierto (arriba) y error puntual (abajo) en una misma figura.
...
def graficar_resultados(salida_dir=None):
    ...
    for json_file in archivos_json:
        ...
        x_desc = np.array(resultado.get("x_sim", []))  # En el JSON se llama x_sim, pero lo interpretamos como modelo descubierto
        
        if len(t) == 0 or len(x_real) == 0 or len(x_desc) == 0:
            print("    ERROR: Datos incompletos (t, x_real o x_sim vacíos)")
            continue
        
        # Calcular error puntual: real - descubierto
        error = x_real - x_desc
